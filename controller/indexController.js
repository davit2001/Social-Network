...
class IndexController {
    ...
    async ConfirmRequest(req, res) {
        ...
        try {
            ...
            if (user.friend.indexOf(to) > -1) return;
            user.friend.push(to)
            ...
            UserFriend.save();
            res.json({ message: `Now ${UserFriend.name} is your firend `, imageName: UserFriend.profilePhotos, info:{user:user.name,to}, friendsCount:user.friendRequest.length })
    ...
    async DeleteRequest(req,res) {
        ...
        try {
         ...
         user.friendRequest = UserFriendRequest
         user.save((err)=>{ if(err) console.log(err)})
         res.json({message:`${UserFriend.name} Delete your friend request`,friendsCount:user.friendRequest.length})
        } catch(err) {
           console.log(err)
        }
    }
